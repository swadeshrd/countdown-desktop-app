...
root = Tk()
root.title("Countdown Dektop App")
...
def countdown():
    ...
    if time > 0:
        ...
        if time == 0:
            messagebox.showinfo("Countdown Timer", "Countdown Time Over")
            btn.config(state=NORMAL)
            btn2.config(state=NORMAL)
...
timee = StringVar()
entry = Entry(root, textvariable=timee, font="ariel 15 bold", width=15, relief=SOLID)
